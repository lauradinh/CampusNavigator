...
public class CS400Graph<T> implements GraphADT<T> {
	...
	public boolean removeVertex(T data) {
		...
		// search all vertices for edges targeting removeVertex (destination)
		for (Vertex v : vertices.values()) {
			Edge removeEdge = null;
			for (Edge e : v.edgesLeaving)
				if (e.target == removeVertex)
					removeEdge = e;
			// and remove any such edges that are found (source)
			if (removeEdge != null)
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path source) {
			...
			List<T> copyOfDataSequence = new LinkedList<>();
			for (int i = 0; i < source.dataSequence.size(); i++) {
				copyOfDataSequence.add(source.dataSequence.get(i));
			}
			this.dataSequence = copyOfDataSequence;
			this.end = source.end;
		}

		/**
		 * Extends (adds to the end) the current path by a single edge and vertex. The
		 * distance, dataSequence, and end of this path are all updated as a result.
		 * 
		 * @param edge is the directed edge that is being added to this path
		 */
		public void extend(Edge edge) {
			dataSequence.add(edge.target.data); // adds data to dataSequence
			this.end = edge.target; // changes the end to the new vertex
			distance += edge.weight; // increases the distance
		}
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		if (!vertices.containsKey(start) || !vertices.containsKey(end)) // no vertex contain start or end can be found
			throw new NoSuchElementException("One or both of these vertices are not in the graph");
		Vertex startVertex = vertices.get(start);
		Vertex endVertex = vertices.get(end);
		Path path = new Path(startVertex); // the main path
		List<Vertex> visitedVertices = new LinkedList<>(); // keep track of visited vertices
		PriorityQueue<Path> queue = new PriorityQueue<>();
		while (!path.end.equals(endVertex)) { 
			if (path.end.edgesLeaving == null) { // path cannot be continued
				throw new NoSuchElementException();
			}
			for (Edge e : path.end.edgesLeaving) { 
				Path childPath = new Path(path); 
				childPath.extend(e);
				if (!visitedVertices.contains(childPath.end)) // only adds paths to unvisited vertices
					queue.add(childPath);
			}
			if (queue.isEmpty()) { // no path from start to end can be found
				throw new NoSuchElementException("There is not path between these two vertices");
			}
			if (!visitedVertices.contains(queue.peek().end)) {
				path = queue.peek();
				visitedVertices.add(path.end);
			}
			queue.remove();
		}
		return path;
